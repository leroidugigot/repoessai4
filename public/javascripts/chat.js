...
function addMessage(data) {
    ...
    messageElement.className = `message ${data.username === userInfo.username ? 'own-message' : 'other-message'}`;
    messageElement.dataset.messageId = data._id;

    const messageTime = data.timestamp ? new Date(data.timestamp).toLocaleTimeString() : new Date().toLocaleTimeString();
    
    messageElement.innerHTML = `
        <div class="message-header">
            <img src="${data.user?.avatar || '/images/default-profile.svg'}" class="message-avatar">
            <span class="message-username">${data.username}</span>
            <span class="message-time">${messageTime}</span>
        </div>
        <div class="message-content">${data.content || data.message}</div>
        <div class="message-footer">
            <div class="message-reactions" data-message-id="${data._id}"></div>
            <div class="message-actions">
                <button class="reaction-trigger" onclick="showReactionSelector('${data._id}')">
...
function changeRoom(newRoom) {
    ...
    document.getElementById('current-room-name').textContent = rooms[newRoom]?.name || newRoom;
    document.getElementById('room-description').textContent = rooms[newRoom]?.description || '';
...
function updateUsersList(users) {
    ...
    users.forEach(user => {
        ...
        userElement.innerHTML = `
            <img src="${user.avatar || '/images/default-profile.svg'}" class="user-avatar-small">
...
function showReactionSelector(messageId) {
    ...
    selector.innerHTML = quickReactions.map(emoji => 
    ...
    document.addEventListener('click', function closeSelector(e) {
        if (!e.target.closest('.reaction-selector') && !e.target.closest('.reaction-trigger')) {
            selector.remove();
...
function setupSocketEvents() {
    ...
    socket.on('reaction-updated', ({ messageId, reactions }) => updateMessageReactions(messageId, reactions));
    
    socket.on('user-connected', (data) => {
        addSystemMessage(`${data.username} a rejoint ${rooms[data.room]?.name || data.room}`);
        updateUsersList(data.users);
    });

    socket.on('user-disconnected', (data) => {
        addSystemMessage(`${data.username} a quitté ${rooms[data.room]?.name || data.room}`);
        updateUsersList(data.users);
    });
